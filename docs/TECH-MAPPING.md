...
| **robots.txt 解析**（urllib.robotparser，Python 内置） | **合规检查**：抓取前确认目标站点允许 | 3.2、3.11 | ⬜ |
...
| 要求 | 做法 |
| --- | --- |
| 只抓 `robots.txt` 允许的路径 | 用 `urllib.robotparser` 先检查 |
| 控制频率 | 每域名 ≥1 秒/次，避免被判定为攻击 |
| 记录来源 | 每条数据存 `source_url` + `crawled_at` + 许可说明 |
| 不用受版权保护的付费资料 | 只抓官方公开信息 + 开放许可内容 |
| 建立 `docs/DATA-SOURCES.md` | 逐条登记来源与许可，答辩时可直接说明数据合规性 |
