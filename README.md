...
> **当前阶段：脚手架已就绪，业务功能待开发。**
> 目录分层、开发约定、前后端连通链路（前端 → 代理 → 后端 → MySQL）已经搭好；
> 仓库里的「注册登录 / AI 问答 / 学习计划」是**照葫芦画瓢用的参考示例**，不是最终产品形态。
> 新同学请先读 **[docs/SCAFFOLD.md](docs/SCAFFOLD.md)**（脚手架使用说明：怎么加自己的模块）。

- **项目定位**：面向考研学生的 AI 陪伴式导学平台
- **规划功能**：RAG 问答、用户画像、个性化规划、推荐评分、多模态问答（**均待开发**）
...
| 模块 | 状态 |
| --- | --- |
| 用户（注册 / 登录 / 个人信息） | 🟡 参考示例，可直接用 |
| AI 对话（会话 / 消息） | 🟡 参考示例；AI 回答目前是关键词规则，**未接入大模型** |
| 学习规划（计划 / 任务 / 统计） | 🟡 参考示例 |
| RAG 问答 / 用户画像 / 推荐评分 / 多模态 | ❌ 待开发 |
| 测试、CI、生产部署 | ❌ 待开发 |
...
- **[docs/SCAFFOLD.md](docs/SCAFFOLD.md)** —— 脚手架使用说明：目录约定、怎么新增一个模块（含完整代码示例）、自检命令、常见坑
- [kaoyan-backend/README.md](kaoyan-backend/README.md) —— 后端开发说明
- [kaoyan-frontend/README.md](kaoyan-frontend/README.md) —— 前端开发说明
